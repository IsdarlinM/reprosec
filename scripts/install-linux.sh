...
if [ -n "$SRIC_SOURCE" ] && [ -f "$SRIC_SOURCE/pyproject.toml" ]; then
  ...
  if ! "$VENV/bin/python" -m pip install -c "$CONSTRAINTS" 'sric-core>=0.4,<0.5'; then
    echo "SRIC Core 0.4.x is required. Clone sric-core next to this repository or set SRIC_CORE_SOURCE=/path/to/sric-core." >&2
    exit 3
  fi
fi
