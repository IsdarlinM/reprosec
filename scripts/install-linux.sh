#!/usr/bin/env sh
set -eu
PROJECT="ReproSec Capsule"; CMD="reprosec"; INSTALL_ROOT="${HOME}/.local/share/reprosec"; VENV="$INSTALL_ROOT/venv"; BIN_DIR="${HOME}/.local/bin"
...
if [ -n "${SRIC_CORE_SOURCE:-}" ]; then
  ...
  "$VENV/bin/python" -m pip install --upgrade --force-reinstall -c "$CONSTRAINTS" "$SRIC_CORE_SOURCE" || { echo "Failed to repair SRIC Core." >&2; exit 3; }
else
  [ -f "$FIRST_PARTY" ] || { echo "Missing first-party dependency manifest: $FIRST_PARTY" >&2; exit 3; }
  "$VENV/bin/python" -m pip install --upgrade --force-reinstall -c "$CONSTRAINTS" -r "$FIRST_PARTY" || { echo "Failed to repair Sentinel Forge first-party dependencies." >&2; exit 3; }
fi
"$VENV/bin/python" -m pip install --upgrade --force-reinstall -c "$CONSTRAINTS" "$REPO_ROOT" || { echo "Installation failed after dependency bootstrap." >&2; exit 3; }
