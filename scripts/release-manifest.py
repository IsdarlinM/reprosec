...
def main() -> int:
    ...
    if wheel.suffix != ".whl": raise SystemExit("release artifact must be a .whl")
    ...
    unsigned = {"product":args.product,"version":args.version,"artifact":args.artifact_url,"sha256":hashlib.sha256(wheel.read_bytes()).hexdigest()}
    payload=(json.dumps(unsigned,sort_keys=True,separators=(",", ":"))+"\n").encode(); manifest=dict(unsigned); manifest["signature"]=base64.b64encode(key.sign(payload)).decode("ascii")
