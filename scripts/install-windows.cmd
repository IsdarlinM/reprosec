...
if defined SRIC_SOURCE ("%VENV%\Scripts\python.exe" -m pip install --upgrade -c "%CONSTRAINTS%" "%SRIC_SOURCE%" || exit /b 3) else ("%VENV%\Scripts\python.exe" -m pip install -c "%CONSTRAINTS%" "sric-core>=0.3,<0.4" || exit /b 3)
