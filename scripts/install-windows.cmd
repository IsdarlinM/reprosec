...
if defined SRIC_CORE_SOURCE (
  ...
  "%VENV%\Scripts\python.exe" -m pip install --upgrade -c "%CONSTRAINTS%" "%SRIC_CORE_SOURCE%" "%REPO_ROOT%" || (echo Atomic ReproSec/SRIC installation failed.& exit /b 3)
) else (
  if not exist "%FIRST_PARTY%" (echo Missing first-party dependency manifest: %FIRST_PARTY%& exit /b 3)
  "%VENV%\Scripts\python.exe" -m pip install --upgrade -c "%CONSTRAINTS%" -r "%FIRST_PARTY%" "%REPO_ROOT%" || (echo Atomic ReproSec/SRIC installation failed.& exit /b 3)
)
"%VENV%\Scripts\python.exe" -m pip check || (echo Installed dependency graph is inconsistent.& exit /b 3)
"%VENV%\Scripts\python.exe" -c "import importlib.metadata as m; import sric.web_console, sric.web_workbench, sric.web_catalog; v=tuple(int(x) for x in m.version('sric-core').split('.')[:3]); raise SystemExit(0 if (0,5,11)<=v<(0,6,0) else 1)" || (echo SRIC Core runtime integrity check failed. Required ^>=0.5.11,^<0.6.& exit /b 3)
