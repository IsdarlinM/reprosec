...
from .capsule import add_assertion, add_request, add_response, add_workflow_step, build_manifest, initialize_directory
...
def doctor(
    ...
    """Check runtime, exact SRIC feature compatibility and safe defaults."""
    import os
    import socket
    import sys
    import cryptography
    ...
    payload: dict[str, object] = {
        "ok": sys.version_info >= (3, 11) and runtime.compatible,
    ...
    if network:
        proxy_vars = {key: bool(os.getenv(key)) for key in ("HTTP_PROXY", "HTTPS_PROXY", "ALL_PROXY", "NO_PROXY")}
        try:
...
def update(
    ...
    """Legacy custom-channel update command retained for compatibility."""
    import os

    source = manifest or os.getenv("REPROSEC_RELEASE_MANIFEST_URL")
    key = public_key or (Path(os.environ["REPROSEC_RELEASE_PUBLIC_KEY"]) if os.getenv("REPROSEC_RELEASE_PUBLIC_KEY") else None)
    if not source or key is None:
        typer.echo("No trusted release channel configured. Provide --manifest and --public-key, or REPROSEC_RELEASE_MANIFEST_URL/REPROSEC_RELEASE_PUBLIC_KEY.", err=True)
        raise typer.Exit(2)
    try:
        status = perform_update(manifest_source=source, public_key_path=key, expected_product="reprosec", current_version=__version__, check_only=check)
...
@app.command("web")
def web(host: str = typer.Option("127.0.0.1", "--host"), port: int = typer.Option(8770, "--port", min=1, max=65535)) -> None:
    """Run the local API. Non-loopback binding remains disabled until authenticated TLS mode is implemented."""
    if host not in {"127.0.0.1", "localhost", "::1"}:
        typer.echo("Non-loopback binding denied: authenticated TLS mode is not configured.", err=True)
        raise typer.Exit(4)
    import uvicorn
...
def demo(output: Path = typer.Option(Path("reprosec-demo"), "--output")) -> None:
    ...
    a = RequestRecord(method="GET", url="https://lab.invalid/doc/123", headers=[], source="user_input")
    add_request(output, a)
    add_workflow_step(output, WorkflowStep(actor="Owner", request_id=a.request_id, state="OBSERVED"))
    b = RequestRecord(method="GET", url="https://lab.invalid/doc/123", headers=[], source="user_input")
    add_request(output, b)
    add_workflow_step(output, WorkflowStep(actor="OtherUser", request_id=b.request_id, depends_on=[a.request_id], state="HYPOTHESIS"))
    add_response(output, ResponseRecord(request_id=a.request_id, status_code=200, body='{"id":123,"owner":"A"}'))
    add_assertion(output, AssertionSpec(request_id=a.request_id, kind="status_code", expected="200"))
    build_manifest(output)
    typer.echo(json.dumps({"demo": str(output), "capsule_id": meta.capsule_id, "network_requests_made": 0}, indent=2))
